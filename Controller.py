class Controller:
    ...
    @classmethod
    def __get_patterns_row(cls, game_state, pattern_dict, is_black):
        size = game_state.size
        # matrix = game_state.matrix
        for i in range(cls.board_size):
            cls.__get_patterns(game_state[i], pattern_dict, is_black)

    @classmethod
    def __get_patterns_col(cls, game_state, pattern_dict, is_black):
        size = game_state.size
        # matrix = board.matrix
        for i in range(size):
            cls.__get_patterns(game_state[:, i], pattern_dict, is_black)
    ...
    @classmethod
    def __get_patterns_diagonal(cls, game_state, pattern_dict, is_black):
        size = cls.board_size
        matrix1 = game_state.matrix
        matrix2 = matrix1[::-1, :]
    ...
    @classmethod
    def evaluate(cls, game_state, is_black_turn: bool):
        cls.evaluation_count += 1
        black_score = cls.get_score(game_state, True, is_black_turn)
        white_score = cls.get_score(game_state, False, is_black_turn)
        if black_score == 0: black_score = 1.0
        return white_score / black_score

    @classmethod
    def get_score(cls, game_state, is_black: bool, is_black_turn: bool):
        pattern_dict = {}
        cls.__get_patterns_row(game_state, pattern_dict, is_black)
        cls.__get_patterns_col(game_state, pattern_dict, is_black)
        cls.__get_patterns_diagonal(game_state, pattern_dict, is_black)
    ...
    @classmethod
    def find_next_move(cls, game_state, depth, is_black):
